...
"""
AI2BIZ Telegram Bot - VERSION V7.2 FINAL
...
# gspread может быть не установлен на сервере.
# Чтобы бот все равно запускался, оборачиваем импорт в try/except.
try:
    import gspread
    GSPREAD_AVAILABLE = True
except ImportError:
    print("⚠️ Модуль gspread не найден. Google Sheets будет отключен.")
...
def safe_send_message(chat_id, text, **kwargs):
    ...
    parse_mode не используем, чтобы исключить ошибки
    'can't parse entities'.
    Вместо этого используем ручное форматирование текста
    (звездочки и эмодзи) как обычные символы.
    """
    try:
...
def save_to_google_sheets(sheet_name, row_data):
    ...
    if not google_sheets:
        print(f"⚠️ Google Sheets не подключена, пропускаю сохранение в {sheet_name}.")
        return False
    ...
    try:
        ...
        print(f"✅ Сохранено в лист '{sheet_name}'.")
        return True
    except Exception as e:
...
def save_lead_files(user_id, lead_data):
    """Сохраняет лид, запросивший файлы, в лист 'Leads Files'."""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
...
def save_lead_consultation(user_id, lead_data):
    """Сохраняет лид консультации в лист 'Leads Consultation'."""
    timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    segment = _calc_segment(lead_data.get("revenue"))
...
def notify_admin_consultation(lead_data):
    """Отправляет уведомление администратору о заявке."""
    if ADMIN_CHAT_ID == 0:
    ...
    notification = (
        ...
        f"Срок работы бизнеса: {lead_data.get('business_duration')}\n"
        ...
        f"Предпочитаемое время: {lead_data.get('zoom_time')}\n"
        f"Сегмент: {segment}\n"
...
def save_message_history(user_id, message_id):
    """Сохраняет ID сообщения пользователя/бота."""
...
def delete_messages_after_welcome(chat_id, user_id):
    """Удаляет сообщения после приветственного."""
    ...
    print(f"🧹 Удалено сообщений после приветствия: {deleted}")
...
def process_cancel_command(message):
    """Обрабатывает команду /cancel из любого состояния."""
...
def process_help_command(message):
    """Обрабатывает команду /help из любого состояния."""
    user_id = message.from_user.id
    chat_id = message.chat.id
    ...
    help_text = (
        "❓ *Поддержка AI2BIZ*\n\n"
        "Если что-то пошло не так или есть вопросы по материалам и автоматизации:\n\n"
        "📩 Напишите нашему специалисту: *@glore4*\n\n"
        "Мы стараемся отвечать в течение *1 часа* в рабочее время.\n\n"
...
def check_for_commands(message):
    """Проверяет, была ли введена /cancel или /help."""
    if not message.text:
        return False
...
def send_welcome_internal(message):
    ...
    welcome_text = (
        f"Привет, {user_name} 👋\n\n"
        "*Я бот AI2BIZ.* Моя задача — помочь тебе:\n"
        "• получить материалы по *автоматизации продаж*;\n"
        "• записаться на *бесплатную консультацию* со специалистом.\n\n"
        "Что я умею:\n"
        "1️⃣ Отправлять *практические материалы* по автоматизации бизнеса.\n"
        "2️⃣ Записывать тебя на *консультацию* с экспертом AI2BIZ.\n\n"
        "Чем полезны материалы и консультация:\n"
        "• помогают увеличить *конверсию на 150–300%*;\n"
        "• сокращают время обработки лидов в *5 раз*;\n"
        "• позволяют *перестать терять до 50% заявок*.\n\n"
        "Как продолжить:\n"
        "• напиши *«файлы»* — чтобы получить материалы;\n"
        "• напиши *«консультация»* — чтобы записаться на созвон;\n"
        "• команда */cancel* — вернуться в главное меню;\n"
        "• команда */help* — связаться с поддержкой."
    )
...
def handle_message(message):
    ...
    if any(word in text for word in ["файлы", "материал", "документ", "pdf"]):
        subscription_text = (
            "Чтобы получить материалы, нужно подписаться на наш канал *AI2BIZ* 📣\n\n"
            f"Канал: *@{CHANNEL_NAME}*\n\n"
            "Зачем это нужно:\n"
            "• там мы делимся *кейcами* по автоматизации продаж;\n"
            "• показываем, как получать *больше заявок при той же рекламе*;\n"
            "• публикуем реальные примеры *роста выручки на 150–300%*.\n\n"
    ...
    if any(
        ...
        consultation_text = (
            "Отлично, давай запишем тебя на *консультацию AI2BIZ* 📞\n\n"
            "На созвоне разберем:\n"
            "• текущую воронку и точки потерь заявок;\n"
            "• потенциал автоматизации именно в твоем бизнесе;\n"
            "• примерный *прогноз роста конверсии и выручки*.\n\n"
            "Для начала напиши, пожалуйста, *как тебя зовут*."
        )
...
def handle_subscription(call):
    ...
    bot.answer_callback_query(call.id, "Спасибо, продолжим!")
    ...
    file_selection_text = (
        "Спасибо за подписку на канал *AI2BIZ* ✅\n\n"
        "Теперь выбери, какой материал тебе нужен в первую очередь:\n\n"
        "• *«5 ошибок менеджеров»* — о том, где обычно теряется до 50% лидов;\n"
        "• *«Чек-лист»* — быстрая диагностика, теряете ли вы заявки и деньги."
    )
...
def handle_file_selection(message, user_id):
    ...
    form_text = (
        "Перед тем как отправить файл, давай заполним короткую *анкету*.\n"
        "Это займет не больше *1 минуты* и поможет понять контекст бизнеса.\n\n"
...
def ask_files_name_check(message, user_id):
    ...
    if not is_valid_name(name):
        error_text = (
            "Имя выглядит некорректно.\n\n"
            "Пожалуйста, введи имя длиной от *2 до 50 символов*."
        )
    ...
    duration_text = "Сколько времени *работает твой бизнес*?"
    ...
    if msg:
        ...
        bot.register_next_step_handler(
            msg, ask_files_business_duration, user_id
        )
...
def ask_files_business_duration(message, user_id):
    ...
    telegram_text = (
        "Оставь, пожалуйста, твой *Telegram*.\n\n"
        "Формат:\n"
        "• *@username* или\n"
        "• *https://t.me/username*."
    )
    ...
    if msg:
        save_message_history(user_id, msg.message_id)
        bot.register_next_step_handler(
            msg, ask_files_telegram_check, user_id
        )
...
def ask_files_telegram_check(message, user_id):
    ...
    business_text = (
        "Расскажи пару слов о *своем бизнесе*:\n\n"
        "• ниша или индустрия;\n"
        "• основной продукт/услуга;\n"
        "• ключевые проблемы в продажах."
    )
...
def ask_files_business(message, user_id):
    ...
    revenue_text = "Какая *выручка в месяц* сейчас?"
...
def finish_form_files(message, user_id):
    ...
    log_action(
        user_id,
        app_data.get("name"),
        "FORM_SUBMITTED_FILES",
        f"Файл: {app_data.get('file_type')}",
    )

    sending_text = (
        "Спасибо, анкета заполнена ✅\n\n"
        "Сейчас отправлю тебе выбранный материал."
    )
    msg = safe_send_message(
    ...
    try:
        if app_data.get("file_type") == "5_mistakes":
            ...
            file_description = (
                "Материал: *«5 ошибок менеджеров, из-за которых теряется до 50% лидов»*\n\n"
                "Внутри:\n"
                "• где именно теряются заявки;\n"
                "• как не терять лидов из-за скорости ответа;\n"
                "• почему без автоматизации система продаж «сыплется».\n\n"
                "После изучения материала рекомендую сделать следующий шаг — "
                "записаться на *консультацию*, чтобы разобрать ситуацию в вашем бизнесе."
            )
        else:
            file_url = FILE_CHECKLIST
            file_description = (
                "Материал: *«10 способов понять, теряете ли вы заявки»*\n\n"
                "Этот чек-лист поможет за 10–15 минут:\n"
                "• найти слабые места в воронке;\n"
                "• оценить, сколько денег теряется каждый месяц;\n"
                "• получить список точек для улучшения.\n\n"
                "После прохождения чек-листа можно записаться на *консультацию*, "
                "чтобы обсудить варианты роста."
            )
        ...
        log_action(
            ...
            app_data.get("name"),
            "DOWNLOAD_FILES",
            f"Получен файл: {app_data.get('file_type')}",
        )

        consultation_offer = (
            "Файл отправлен ✅\n\n"
            "Если хотите не просто посмотреть материал, а получить *конкретный план* "
            "по росту заявок и выручки — запишитесь на *бесплатную консультацию*.\n\n"
            "Напишите слово *«консультация»*, и я задам несколько уточняющих вопросов "
            "для подготовки к созвону."
        )
        ...
        error_msg = safe_send_message(
            chat_id, "Произошла ошибка при отправке файла. Попробуйте чуть позже."
        )
        if error_msg:
...
def ask_consultation_business_duration(message, user_id):
    ...
    telegram_text = (
        "Оставь, пожалуйста, твой *Telegram* для связи.\n\n"
        "Формат:\n"
        "• *@username* или\n"
        "• *https://t.me/username*."
    )
...
def ask_consultation_email_check(message, user_id):
    ...
    business_text = (
        "Расскажи, пожалуйста, немного о *бизнесе и продажах*:\n\n"
        "• ниша и продукт;\n"
        "• какая выручка в месяц сейчас;\n"
        "• какие основные проблемы видишь в воронке."
    )
    ...
    if msg:
        save_message_history(user_id, msg.message_id)
        bot.register_next_step_handler(
            msg, ask_consultation_business, user_id
        )
...
def ask_consultation_business(message, user_id):
    ...
    if msg:
        save_message_history(user_id, msg.message_id)
        bot.register_next_step_handler(
            msg, ask_consultation_revenue, user_id
        )
...
def ask_consultation_revenue(message, user_id):
    ...
    participants_text = (
        ...
        "Это помогает подобрать формат встречи."
    )
    ...
    if msg:
        save_message_history(user_id, msg.message_id)
        bot.register_next_step_handler(
            msg, ask_consultation_participants, user_id
        )
...
def ask_consultation_participants(message, user_id):
    ...
    time_text = (
        "Когда примерно *удобно выйти в Zoom*?\n\n"
        "Можно выбрать диапазон, а менеджер уточнит точное время."
    )
    ...
    if msg:
        save_message_history(user_id, msg.message_id)
        bot.register_next_step_handler(
            msg, finish_form_consultation, user_id
        )
...
def finish_form_consultation(message, user_id):
    ...
    confirmation = (
        ...
        f"• Срок работы бизнеса: *{app_data.get('business_duration')}*\n"
        f"• Telegram: *{app_data.get('telegram')}*\n"
        f"• Email: *{app_data.get('email')}*\n"
        f"• Выручка: *{app_data.get('revenue')}*\n"
        f"• Кто будет на созвоне: *{app_data.get('participants')}*\n"
        f"• Предпочитаемое время: *{app_data.get('zoom_time')}*\n\n"
        "Что дальше:\n"
        "• наш специалист свяжется с тобой в Telegram в течение *рабочего часа*;\n"
        "• согласует точное время Zoom-встречи;\n"
        "• подготовит *предварительный план роста* под твой бизнес.\n\n"
        "Спасибо, что выбрал *AI2BIZ*. Также не забывай про канал *@it_ai2biz* — "
        "там много практики по автоматизации продаж."
    )
...
def index():
    return (
        "<h1>AI2BIZ Telegram Bot v7.2</h1>"
        "<p><strong>Статус:</strong> Активен</p>"
        "<p><strong>Интеграция:</strong> Google Sheets (если gspread установлен)</p>"
    )
...
if __name__ == "__main__":
    print("✅ AI2BIZ Bot v7.2 запущен.")
    if not GSPREAD_AVAILABLE:
        print(
            "ℹ️ Для включения Google Sheets добавьте в requirements.txt строку "
            "'gspread' и redeploy."
        )
